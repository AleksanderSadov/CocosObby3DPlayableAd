...
1. Не все Feature Cropping можно проверить в рантайме в редакторе, видимо подтянет некоторые все равно, нужно проверять в превью браузера и в билде
1. Сейчас не было времени в точности вести учет, пока приведу примерный расклад:
    ...
    1. Остальное всё по мелочи убрал методом исключения убирая и смотря не будет ли ошибок, остались только базово нужные
